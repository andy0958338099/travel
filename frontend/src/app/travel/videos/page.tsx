...
function getEmbedUrl(url: string): { embedUrl: string; platform: Video['platform'] } {
  ...
  if (ytMatch) return { embedUrl: `https://www.youtube.com/embed/${ytMatch[1]}?rel=0`, platform: 'youtube' };

  const biliMatch = url.match(/bilibili\.com\/video\/(BV[\w]+)/);
  if (biliMatch) return { embedUrl: `https://player.bilibili.com/player.html?bvid=${biliMatch[1]}&autoplay=0`, platform: 'bilibili' };

  const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
  if (vimeoMatch) return { embedUrl: `https://player.vimeo.com/video/${vimeoMatch[1]}`, platform: 'vimeo' };

  if (url.match(/\.(mp4|webm|mov)(\?.*)?$/i)) return { embedUrl: url, platform: 'direct' };
...
const CATEGORIES = ['全部', '行前準備', '景點介紹', '美食推薦', '交通參考', '住宿推薦'];
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-1">影片標題</label>
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
            placeholder="自動從網址抓取，也可自行填寫" className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.slice(1).map((cat) => (
              <button key={cat} type="button" onClick={() => setCategory(cat)}
                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${category === cat ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <button type="submit" disabled={loading}
          className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white font-semibold py-2.5 rounded-xl transition-colors">
          {loading ? '儲存中...' : '分享影片'}
        </button>
      </form>
    </div>
...
function VideoCard({
  ...
  const handleDeleteVideo = async () => {
    if (!adminUnlocked) { setShowDeleteConfirm(true); return; }
    setDeleting(true);
    ...
    <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          <span className={`text-xs px-2 py-0.5 rounded-full text-white font-medium flex-shrink-0 ${CATEGORY_COLORS[video.category] ?? 'bg-gray-500'}`}>
            {video.category}
          </span>
      ...
      {showComments && (
        <div className="border-t border-gray-100 px-4 py-4 bg-gray-50">
          {comments.length === 0 && <p className="text-center text-gray-400 text-sm py-4">尚無留言，搶沙發！</p>}
              ...
              <div key={c.id} className="flex gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <button onClick={async () => { await deleteCommentFromDb(c.id); onDeleteComment(c.id); }}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <input type="password" id="admin-pwd-video" placeholder="管理員密碼"
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 bg-gray-100 text-gray-600 py-2 rounded-xl text-sm font-medium hover:bg-gray-200">取消</button>
              <button onClick={() => {
...
export default function VideosPage() {
  ...
  const loadCommentsForVideo = useCallback(async (videoId: string) => {
    ...
    setCommentsMap((prev) => ({ ...prev, [videoId]: data }));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
        {showAddForm && (
          <AddVideoForm onAdd={(v) => { setVideos((prev) => [v, ...prev]); setShowAddForm(false); }} onCancel={() => setShowAddForm(false)} />
        )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-4 py-3 mb-6 flex flex-wrap items-center gap-x-4 gap-y-2">
          ...
          {members.map((m) => (
            <span key={m.id} className="flex items-center gap-1.5">
              <span className={`w-5 h-5 rounded-full ${m.color} flex items-center justify-center text-white text-[10px] font-bold`}>{m.name.charAt(0)}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {filtered.map((video) => (
              <VideoCard
                ...
                onRefreshComments={() => loadCommentsForVideo(video.id)}
              />
            ))}
          </div>
        )}
