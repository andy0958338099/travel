...
const STORIES = [
  ...
  },
  {
    ...
    content: [
      {
        subtitle: '什麼是「宮宴」——沉浸式餐飲體驗',
        text: '杭州「宮宴」（Gongyan Palace Restaurant）是 2018 年在杭州西湖文化廣場附近開業的沉浸式宮廷主題餐廳，融合「古裝換裝 + 宮廷場景 + 宋代宮廷菜 + 演繹式服務」於一身。入場前服務員會引導賓客換上漢服（男女皆有不同款式可選），隨後在雕樑畫棟、宮燈搖曳的「宮殿」內用餐，侍者身穿宮女／太監服飾以「娘娘」「萬福」等古禮問候，是中國近五年「新文娛＋餐飲」業態的代表案例之一。⚠️ 中堂業界常識值：歷史年代、店家具體開幕年份請以官方公告為準。',
      },
      {
        subtitle: '宮宴跟一般「古裝體驗餐廳」有何不同',
        text: '⚠️ 中堂業界常識值：杭州目前有數家類似定位的店家（如「南宋御園」「穿越宋朝」「宮廷御膳」等品牌），價格、菜單、規模差異很大。宮宴的特色據公開資訊是「宋代宮廷菜 + 沉浸式表演 + 中大型宴會廳」三合一，鎖定 8-20 人團客或家庭聚餐。聖上若想體驗「一個人也能玩」的小型古裝餐廳，建議另查詢。⚠️ 店家業態變動快，價格與菜品請出發前到宮宴官方微信公眾號或小紅書帳號當下確認。',
      },
      {
        subtitle: '體驗流程（6 步）',
        text: '⚠️ 中堂業界常識值，流程可能因店家或時段不同而異：(1) 微信／電話預約，確認場次時段（通常有午宴 11:30、晚宴 18:30 兩場）。(2) 提前 30 分鐘到場，領取漢服（內含上衣、下裙、腰帶、鞋，部分含頭飾）。(3) 換裝、整理儀容，店家提供妝髮服務（可能加價）。(4) 入場入座，菜單依套餐分級（基礎 / 進階 / 尊享），通常含 8-12 道菜。(5) 邊用餐邊觀賞表演（古樂、宮廷舞、詩詞吟誦，部分場次有「皇后出巡」互動）。(6) 用餐結束後換回便服、拍照打卡、可能獲贈紀念品。整套體驗約 2-3 小時。',
      },
      {
        subtitle: '宋代宮廷菜吃什麼',
        text: '⚠️ 菜單隨店家與季節調整，以下為常見菜品類型：開胃冷盤（南宋鹵味、糟鵝、醉蝦）、主菜（東坡肉、宋嫂魚羹、西湖醋魚、龍井蝦仁、叫化雞、蜜汁火方、菊花豆腐）、點心（定勝糕、荷花酥、艾草青團、宋代果子）、甜湯（冰糖湘蓮、銀耳羹）。⚠️ 中堂業界常識值：宋代「宮廷御膳」史料有限，現代餐廳多是「仿宋菜」——以宋代名菜為靈感、用現代食材重現，不會跟宋代完全一致。聖上有飲食禁忌（素食、海鮮過敏等）請預約時先告知店家。',
      },
      {
        subtitle: '價格與預算',
        text: '⚠️ 估算非保證：杭州宮宴人均消費約 ¥300-500（人民幣），依套餐等級浮動。addons：妝髮加價約 ¥50-100、漢服升級款加價約 ¥50-200。聖上 Day 7 行程預算 NT$2500 / 人（含運河遊船 + 宮宴 + 大馬弄早餐等），換算後尚有 buffer。若想升級 VIP 套餐（含獨立包廂、更高級漢服、互動表演更密集）預算請拉到 ¥800-1200 / 人。⚠️ 價格隨節假日浮動大（春節、十一長假可上調 20-50%），請預約時確認當日報價。',
      },
      {
        subtitle: '拍照攻略 & 文化小知識',
        text: '宮宴場景設計講究，拍照打卡點推薦：(1) 入口宮門（紅牆金瓦）— 必拍「娘娘駕到」劇照。(2) 主宴區（宮燈 + 雕花窗）— 餐桌合影。(3) 換裝區鏡子前 — 漢服全身鏡。(4) 部分店家設「御花園」小院 — 夜景燈籠 + 古樹。文化小知識：宋代宮廷宴席分「常餐」（兩餐制）與「宴饗」（節慶大宴），宮宴套餐形式偏向後者。菜色命名常引用《夢粱錄》《武林舊事》等南宋筆記，是中國「食學」研究的當代延伸。⚠️ 以上文化敘述為中堂業界常識值，請查《宋史·飲食志》當代學術研究當下確認。',
      },
      {
        subtitle: '前往交通 & 與 Day 7 行程整合',
        text: '杭州宮宴位於西湖文化廣場附近，地鐵 1 號線「龍翔橋」站步行約 10 分鐘，或 19 號線「西湖文化廣場」站步行 5 分鐘。聖上 Day 7 行程：上午河坊街 → 中午大馬弄早餐或知味觀中餐 → 下午京杭大運河遊船（拱宸橋登船）→ 傍晚抵達宮宴用餐 → 晚上可加武林夜市吃點心。⚠️ 時間節奏建議：河坊街到運河遊船碼頭約 30 分鐘車程，運河遊船 1.5 小時，下船後到宮宴約 20 分鐘，整體傍晚 18:00 用餐時間合理。請提前 1-3 天預約宮宴。',
      },
      {
        subtitle: '替代方案（預約不到宮宴時）',
        text: '⚠️ 中堂業界常識值：若宮宴當日預約已滿，可考慮：(a) 「樓外樓」西湖店（150 年老字號，正宗杭幫菜，¥150-300 / 人）— 非沉浸式但文化底蘊深。(b) 「知味觀」湖濱店（百年老字號，小籠包與杭幫菜）— 平價且品質穩定。(c) 「外婆家」系列（連鎖杭幫菜，¥80-150 / 人）— 平價大眾化。(d) 河坊街「天興樓」或「皇飯兒」— 仿宋菜小店，可穿古裝但無演繹服務。聖上若 Day 7 宮宴預約不到，回飯店前在河坊街吃晚飯也是合理替代。',
      },
    ],
  },
];
