...
LOG_PATH = "/tmp/manga-batch-v3.log"
WORKER = "https://jiangnan-trip.andy0958338099.workers.dev"
MAX_RETRY = 1  # fail fast — 不要再拖 4 小時
COOLDOWN_AFTER_FAIL = 12
DEFAULT_COOLDOWN = 32  # 略低於 waitUntil 30s 上限 — 期待 MiniMax 25s + upload 4s + PATCH 1s = 30s 邊緣
...
def main():
    log("=" * 60)
    log("v3 監視大臣啟動（衝突感 prompt + 只補缺 panel + fail fast）")
    log("=" * 60)
